...
export function CampaignTable({
  ...
  const columns = [
    ...
    columnHelper.accessor("category", {
      cell: (data) => CAMPAIGN_CATEGORY_LABEL[data.getValue()],
      header: "상세 분류",
    }),
    ...
    columnHelper.accessor("exposureTags", {
      cell: (data) => {
        const exposureTag = data.getValue()
          ? EXPOSURE_TAG_LABEL[data.getValue()[0]]
          : "-";
        return exposureTag;
      },
      header: "홈 진열 상태",
  ...
  //  체험단 상품 클릭 핸들러
  const clickCampaign = (row: Row<CampaignItem>) => {
    const id = row.original.id;
    router.push(
      `${CAMPAIGN_PRODUCT_DETAIL_PAGE.replace("[id]", id)}?prev=${prev}`,
    );
  };

  // 체험단 상품 생성 핸들러
  const clickCreateCampaign = () => {
    router.push(CAMPAIGN_PRODUCT_NEW_PAGE);
  };
  ...
  return (
    <div className="flex flex-col gap-[15px] overflow-x-auto">
      <div className="flex items-center justify-between">
        ...
        />
        <CreateCtaButton onClick={clickCreateCampaign} />
      </div>
      <Table className="">
        <TableHeader>
