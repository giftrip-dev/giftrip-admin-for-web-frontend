...
import { RESERVATION_CAMPAIGN_DETAIL_PAGE } from "@/constants/path";
...
export function CampaignTable({
  ...
  const columnHelper = createColumnHelper<CampaignReservationItem>();
  const columns = [
  ...
  // 체험단 상품 클릭 핸들러
  const clickCampaignReservation = (row: Row<CampaignReservationItem>) => {
    const id = row.original.id;
    router.push(`${RESERVATION_CAMPAIGN_DETAIL_PAGE.replace("[id]", id)}`);
  };
            ...
            table.getRowModel().rows.map((row, rowIndex) => (
              ...
              >
                {row.getVisibleCells().map((cell, cellIndex) => (
                  <TableCell
                    onClick={() => clickCampaignReservation(row)}
                    size="md"
