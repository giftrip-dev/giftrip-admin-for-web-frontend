...
import { RESERVATION_ACCOMMODATION_DETAIL_PAGE } from "@/constants/path";
...
export function AccommodationTable({
  ...
  const columnHelper = createColumnHelper<AccommodationReservationItem>();
  const columns = [
    ...
    columnHelper.accessor("checkInDate", {
      cell: (data) => {
        const checkInDate = formattedDate(data.getValue(), "YYYY-MM-DD");
        const checkOutDate = formattedDate(
          data.row.original.checkOutDate,
          "YYYY-MM-DD",
        );
    ...
    columnHelper.accessor("accommodationMatchingStatus", {
      cell: (data) => (
        <StatusChip
          color={getChipColor(data.getValue())}
          status={ACCOMMODATION_MATCHING_STATUS_LABEL[data.getValue()]}
        />
      ),
      header: "매칭 상태",
    }),
  ...
  // 숙소 예약 상품 클릭 핸들러
  const clickAccommodationProduct = (
    row: Row<AccommodationReservationItem>,
  ) => {
    const id = row.original.id;
    router.push(
      `${RESERVATION_ACCOMMODATION_DETAIL_PAGE.replace("[id]", id)}?prev=${prev}`,
    );
  };
