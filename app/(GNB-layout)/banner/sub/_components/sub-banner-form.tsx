...
import { usePostSubBanner } from "@/hooks/banner/use-post-banner";
import { usePutSubBanner } from "@/hooks/banner/use-put-sub-banner";
import CancelAndCreateButtonBar from "@/app/_components/button/cancel-and-create-button-bar";
import { useRouter } from "next/navigation";
import { SUB_BANNER_DETAIL_PAGE, SUB_BANNER_PAGE } from "@/constants/path";

interface SubBannerFormProps {
  mode: "create" | "edit";
  bannerId?: string;
}

const SubBannerForm = ({ mode, bannerId }: SubBannerFormProps) => {
  const createBanner = usePostSubBanner();
  const updateBanner = usePutSubBanner(bannerId);
  ...
  const clickCancel = () => {
    if (mode === "create") {
      router.push(SUB_BANNER_PAGE);
    } else {
      router.push(SUB_BANNER_DETAIL_PAGE.replace("[id]", bannerId as string));
    }
  };
      ...
      <form onSubmit={onSubmit}>
        ...
        <ShortRow label="배너 이미지" value={""} size="lg" isLastRow>
          <ImageInputField
            form={form}
            name="imageUrl"
            description="이미지를 업로드해주세요 ( 1080*200 )"
