import { BannerItem, SubBannerCreateReq } from "@/app/api/dto/banner";
import useSWR from "swr";
import { toast } from "../use-toast";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import * as z from "zod";
import { subBannerSchema } from "@/schema/banner";
import { useRouter } from "next/navigation";
import { SUB_BANNER_DETAIL_PAGE } from "@/constants/path";
...
export const usePutSubBanner = (bannerId?: string) => {
  ...
  const mutate = async (data: z.infer<typeof subBannerSchema>) => {
    setIsLoading(true);
    try {
      ...
      const reqData: SubBannerCreateReq = {
        title: data.title,
        type: data.type,
      ...
      router.replace(SUB_BANNER_DETAIL_PAGE.replace("[id]", bannerId!));
    } catch (error) {
